...
class PaginatorComponent extends Component
{
    ...
    public function paginate($object, array $settings = [])
    {
        ...
        if (empty($query)) {
			...
			if(isset($options['conditions']) && array_key_exists('between',$options['conditions']) == true) {
				$between = $options['conditions']['between'];
				unset($options['conditions']['between']);
				$query = $object->find($finder, $options)->where(function ($exp, $q) use ($between) {
					return $exp->between($between[0], $between[1], $between[2]);
				});
			} else {
