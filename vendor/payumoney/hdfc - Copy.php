...
/*
    ...
    public function getAction() {
        return $this->hdfcBaseURL . '/transaction/transaction.do?command=initiatePayloadTransaction';
    }
    ...
    public function generateHash($posted = []) {
        ...
        // Hash Sequence
        $hashSequence = "merchant_id|order_id|currency|amount|redirect_url|cancel_url|encRequest";
    ...
    public function send($posted = []) {
        ...
        $posturl = $this->hdfcBaseURL . '/transaction/transaction.do?command=initiatePayloadTransaction';

        $payu_in_args = array(
            'merchant_id'       => $this->merchantKey,
            'redirect_url'      => $posted['surl'],
            'cancel_url'        => $posted['curl'],
        ...
        $payuform .= '<input type="hidden" name="order_id" value="' . $posted['txnid'] . '" />' . "\n";
        ...
        echo '
          ...
          <div class="box">
            Thank you for your order. We are now redirecting you to PayUMoney to make payment.
          </div>
    ...
    public function send($posted = []) {
        ...
        $payuform .= '<input type="hidden" name="udf1" value="' . $posted['udf1'] . '" />' . "\n";
        if(array_key_exists('udf2', $posted))
        {
            $payuform .= '<input type="hidden" name="udf2" value="' . $posted['udf2'] . '" />' . "\n";
        }
        $payuform .= '<input type="hidden" name="hash" value="' . $this->hash . '" />' . "\n";
