...
class CustomersController extends FrontAppController
{	
					...
					if($key=='action') {
						...
						if($blnEditCustomersRights){
							...
							if(!empty($val['status']))
								...
								$temparr['action'].= $this->Userright->linkListProjects(constant('WEB_URL').constant('ADMIN_PATH').'projects/index/'.encode($val['id']),'<i class="fa fa-eye"></i>','','viewRecord',' title="View Projects"')."&nbsp;";	
						}
					}
				}		
			}
	...
	*/
	public function getcustomerprofile() {
		...
		$custData 	= array();
		if(!empty($cus_id)) {
			$custData 	= $this->Customers->find('all', array('conditions'=>array('id'=>$cus_id), 'fields'=>array('name','mobile')))->toArray();
		}		
	...
	*/
	...
	public function makepayment()
	{
		...
		$arrCustomer					= $this->Customers->find('all',array("conditions"=>array('Customers.id'=>$this->ApiToken->customer_id)))->first();
		if (!empty($arrCustomer)) {
			if (ctype_digit($Amount)) {
				$email 			= $arrCustomer['email'];
				$mobile 		= $arrCustomer['mobile'];
				$ValidRequest 	= $this->PaymentRequest->GetPaymentURL($this->ApiToken->customer_id,$email,$mobile,$Amount);
				if (!$ValidRequest) {
					$this->ApiToken->SetAPIResponse('status', 'fail');
					$this->ApiToken->SetAPIResponse('error',$this->PaymentRequest->error_message);
				} else {
					$data = array("weburl"=>$this->PaymentRequest->getTransactionPageUrl());
					$this->ApiToken->SetAPIResponse('status', 'success');
					$this->ApiToken->SetAPIResponse('data',$data);
				}
			} else {
				$this->ApiToken->SetAPIResponse('status', 'fail');
				$this->ApiToken->SetAPIResponse('error',"Please enter valid amount for wallet.");
			}
		} else {
			$this->ApiToken->SetAPIResponse('status', 'fail');
			$this->ApiToken->SetAPIResponse('error',"Invalid Customer details.");
		}
		echo $this->ApiToken->GenerateAPIResponse();
		exit;
	}
	...
	{
		$this->layout		= false;
		$request_id			= $this->request->data('txn_id');
		$payment_status		= $this->request->data('payment_status');
		$arrCondition		= array("PaymentRequest.request_id"=>DBVarconv($request_id));
		$message 			="";
		$arrPaymentRequest 	= $this->PaymentRequest->find("first",array("conditions"=>$arrCondition))->first();;
		if (!empty($arrPaymentRequest)) {
			$TransactionFound 	= $this->CustomerSubscription->find("count",array("conditions"=>array("CustomerSubscription.payment_id"=>$arrPaymentRequest['PaymentRequest']['id']))); 
			if ($TransactionFound <= 0) {
				$arrPaymentRequestDetails = $this->PaymentRequest->getPaymentRequestDetails($arrPaymentRequest['PaymentRequest']['request_id']);
				if (strtoupper($payment_status) == 'F' || !$arrPaymentRequestDetails) {
					$failed_reason = isset($this->PaymentRequest->api_response->reason)?$this->PaymentRequest->api_response->reason:"Error in payment processing. Not valid transaction ID.";
					$this->PaymentRequest->updateAll(
						array(	'PaymentRequest.status' => 0,
								'PaymentRequest.failed_reason' => "'".DBVarconv($failed_reason)."'"),
						array('PaymentRequest.id' => $arrPaymentRequest['PaymentRequest']['id'])
					);
					$message = $failed_reason;
				} else {
					$this->PaymentRequest->updateAll(
						array('PaymentRequest.status' => 1),
						array('PaymentRequest.id' => $arrPaymentRequest['PaymentRequest']['id'])
					);
					$customer_id 		= $arrPaymentRequest['PaymentRequest']['customer_id'];
					$wallet_id 			= $this->CustomerSubscription->SaveWalletDetails($arrPaymentRequest['PaymentRequest'],0);
					$payment_status = "S";
					$message 		= "Transaction done successfully. Points added in your account.";
				}
			} else {
				$payment_status = "F";
				$message 		= "Invalid transaction details. Payment already processed for given transaction details.";
			}
		} else {
			$payment_status = "F";
		...
		$this->ApiToken->SetAPIResponse('status', 'success');
		$this->ApiToken->SetAPIResponse('message',$message);
		echo $this->ApiToken->GenerateAPIResponse();
		exit;
	}
