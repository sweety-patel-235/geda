...
class PhpExcelComponent extends Component {
    ...
    private $defaults = array('extension' => '.xls', 'excelName' => 'ExcelSheet', 'sheet1Name' => 'Sheet1');
...
//$pdfPath = SITE_ROOT_DIR_PATH.'/tmp/'.$fileName;
       // $output = $objWriter->save($pdfPath);
       // echo $fileName."111";
       // exit;
        //$pdfPath = SITE_ROOT_DIR_PATH.'/tmp/'.$fileName;
        //$output = $objWriter->save($pdfPath);
    ...
    public function downloadFile_mail($fileName = null) {
        $objWriter = \PHPExcel_IOFactory::createWriter($this->objPHPExcel, $this->inputFileType);  // writer object
  ...
  /*
     * ************** Style related functions*****************************
     */
    /*     * *****Font colour of a particular cell.Colour code in hex value without "#" ***********  and is_bold TRUE/FALSE */
      function fillCellFont($cellDim, $color,$is_bold=FALSE,$size='11px') {
        $cellBckStyle = array(
            'font' => array(
                'bold' => $is_bold,
                'size' => $size,
        ...
        $this->objWorksheet->getStyle($cellDim)->applyFromArray($cellBckStyle); // font colour
    }
    ...
    function writeMergeCellValue($cell) {
        $this->objWorksheet->mergeCells($cell);
    }
