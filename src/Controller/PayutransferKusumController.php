...
class PayutransferKusumController extends AppController
{
	...
	*/
    public function index($application_form_id='')
    {
		...
		if(PAYMENT_METHOD=='hdfc')
        {
			...
			$hdfc['redirect_url'] 		= Router::url(['controller' => 'PayutransferKusum','action' => 'success'],TRUE); // Success Url
			$hdfc['cancel_url'] 		= Router::url(['controller' => 'PayutransferKusum','action' => 'cancel'],TRUE); 	// Cancel Url
			...
			$ApplicationKusumPaymentRequest 			= TableRegistry::get('ApplicationKusumPaymentRequest');
			$ApplicationRequestEntity 					= $ApplicationKusumPaymentRequest->newEntity();
			...
			$ApplicationKusumPaymentRequest->save($ApplicationRequestEntity);
		...
		$payu['surl'] 			= Router::url(['controller' => 'PayutransferKusum','action' => 'success'],TRUE); // Success Url
		$payu['furl'] 			= Router::url(['controller' => 'PayutransferKusum','action' => 'failure'],TRUE); // Fail Url
		$payu['curl'] 			= Router::url(['controller' => 'PayutransferKusum','action' => 'cancel'],TRUE); 	// Cancel Url
        ...
        if($this->request->data)
		{
           	if(PAYMENT_METHOD=='hdfc')
        	{
	           	require_once(ROOT . DS . 'vendor' . DS . 'hdfc' . DS . 'hdfc.php');
	       	   	...
	       	   	{
	       	   		$response=$this->PayumoneyKusum->savedata_success($arr_pass_data,0);
	       	   	}
	       	   	else
	       	   	{
	       	   		$response=$this->PayumoneyKusum->savedata_failure($arr_pass_data,0);
	       	   		$this->Flash->error('Payment failed.');
                	return $this->redirect(URL_HTTP.'view-applyonline-kusum/'.$this->request->data['udf1']);
	       	   	}
	       	}
	       	else
	       	{
	       		$response=$this->PayumoneyKusum->savedata_success($this->request->data,0);
	       	}
           	if($response==1)
            {
	...
	*/
	...
	{
        if($this->request->data){
        	$response 		= $this->PayumoneyKusum->savedata_failure($this->request->data,0);
