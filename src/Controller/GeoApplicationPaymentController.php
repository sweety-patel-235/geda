 ...
 */
class GeoApplicationPaymentController extends AppController
{
    ...
    public function index()
    {
		...
		if($this->Session->check('Customers')) 
		{
			...
			if($customerId=='')
			{
				...
				return $this->redirect(URL_HTTP.'applications_geo_location/'.$id);
			}
		} else {
		...
		else
		{
			...
			elseif($exist_application_Geo[0]->payment_status!=1)
			{
		...
		if(PAYMENT_METHOD=='hdfc')
        {
			...
			$hdfc['redirect_url'] 		= URL_HTTP.'GeoPayment/success';//Router::url(['controller' => 'GeoApplicationPayment','action' => 'success'],TRUE); // Success Url
			$hdfc['cancel_url'] 		= URL_HTTP.'GeoPayment/cancel';//Router::url(['controller' => 'GeoApplicationPayment','action' => 'cancel'],TRUE); 	// Cancel Url
		...
		if($this->request->data)
		{
           	if(PAYMENT_METHOD=='hdfc')
        	{
	           	require_once(ROOT . DS . 'vendor' . DS . 'hdfc' . DS . 'hdfc.php');
	       	   	...
	       	   	{
	       	   		$response=$this->GeoApplicationPayment->savedata_success($arr_pass_data,0);
	       	   	}
	       	   	else
	       	   	{
	       	   		$response=$this->GeoApplicationPayment->savedata_failure($arr_pass_data,0);
	       	...
	       	// else
	       	// {
	       	// 	$response=$this->GeoApplicationPayment->savedata_success($this->request->data,0);
	       	// }
	       	if($response==1)
            {
                $this->Flash->success('Payment done successfully.');
                //return $this->redirect(URL_HTTP.'applications_geo_location/'.$this->request->data['udf1']);
                return $this->redirect(URL_HTTP.'applications-list');
            }
            else
            {
	...
	*/
	...
	{
        if($this->request->data){
        	$response 		= $this->GeoApplicationPayment->savedata_failure($this->request->data,0);
	...
	{
	if($this->request->data){
		$payuTable = TableRegistry::get('GeoApplicationPayment');
		if(PAYMENT_METHOD=='hdfc')
		{
			require_once(ROOT . DS . 'vendor' . DS . 'hdfc' . DS . 'hdfc.php');
				$objHdfc 			= new Hdfc(Configure::read('HDFC_MERCHANT_KEY'),Configure::read('HDFC_SALT'),Configure::read('HDFC_ACCESS_CODE'), Configure::read('PAYU_SANDBOX'));
