 ...
 */
class PayutransferKusumController extends AppController
{
	...
	*/
    public function index($application_form_id='')
    {
		if($this->Session->check('Customers')) 
		{
			...
			if($customerId=='')
			{
				return $this->redirect(URL_HTTP.'apply-online-kusum-list');
			}
        } else {
	...
	*/
    public function installer_payment($installer_id='')
    {
		...
		else
		{
			...
			$exist_installer	= $Installers->find('all',array('conditions'=>array('id'=>decode($installer_id))))->first();
				
			if(empty($exist_installer))
			{
				 return $this->redirect(URL_HTTP.'installer-registration');
			}
			elseif($exist_installer->payment_status!=1)
			{
				$proceesingFee	= INSTALLER_PAYMENT_FEES;
				$GST   			= INSTALLER_GST_AMOUNT;
			}
			else
			{
				return $this->redirect(URL_HTTP.'installer-registration');
		...
		if(PAYMENT_METHOD=='hdfc')
        {
			...
			$txnId 						= $objHdfc->randomTxnId();
	        $hdfc['order_id'] 			= $txnId;               //Transaction Id
	        $hdfc['redirect_url'] 		= Router::url(['controller' => 'Payutransfer','action' => 'installer_success'],TRUE); // Success Url
	        $hdfc['cancel_url'] 		= Router::url(['controller' => 'Payutransfer','action' => 'installer_cancel'],TRUE); 	// Cancel Url
	        $hdfc['amount'] 			= $proceesingFee + $GST; // Amount
	        $hdfc['language'] 			= 'EN';
	        $hdfc['currency'] 			= 'INR';
	        $hdfc['billing_name'] 		= preg_replace('/[^a-z0-9 ]/i', '',$exist_installer->installer_name);
	        $hdfc['billing_country'] 	= 'India';
	        $hdfc['billing_address'] 	= preg_replace('/[^a-z0-9 ]/i', '',$exist_installer->address);
	        $hdfc['billing_city'] 		= preg_replace('/[^a-z0-9 ]/i', '', $exist_installer->city);
	        $hdfc['billing_state'] 		= preg_replace('/[^a-z0-9 ]/i', '',$exist_installer->state);
	        $hdfc['billing_zip'] 		= $exist_installer->pincode;
	        $hdfc['billing_tel'] 		= $exist_installer->mobile;
	        $hdfc['delivery_name'] 		= preg_replace('/[^a-z0-9 ]/i', '',$exist_installer->installer_name);
	        $hdfc['delivery_country'] 	= 'India';
	        $hdfc['delivery_address'] 	= preg_replace('/[^a-z0-9 ]/i', '',$exist_installer->address);
	        $hdfc['delivery_city'] 		= preg_replace('/[^a-z0-9 ]/i', '', $exist_installer->city);
	        $hdfc['delivery_state'] 	= preg_replace('/[^a-z0-9 ]/i', '',$exist_installer->state);
	        $hdfc['delivery_zip'] 		= $exist_installer->pincode;
	        $hdfc['delivery_tel'] 		= $exist_installer->mobile;
	        $hdfc['merchant_param1'] 	= encode($exist_installer->id);
	        
	        $request_data 								= json_encode($hdfc);
			$InstallerPaymentRequest 					= TableRegistry::get('InstallerPaymentRequest');
			$InstallerRequestEntity 					= $InstallerPaymentRequest->newEntity();
			$InstallerRequestEntity->installer_id 		= $exist_installer->id;
			$InstallerRequestEntity->created 			= $this->NOW();
			$InstallerRequestEntity->modified 			= $this->NOW();
			$InstallerRequestEntity->request_data		= $request_data;
			$InstallerRequestEntity->amount 			= $hdfc['amount'];
			$InstallerPaymentRequest->save($InstallerRequestEntity);
	        $objHdfc->send($hdfc);
    	}
        exit;
    }
	...
	public function installer_cancel()
	{
		if($this->request->data) {
			...
			if ($this->InstallerPayment->save($inspaymentsave)) {
				$InstallerPaymentRequest 		= TableRegistry::get('InstallerPaymentRequest');
				$arrPayment 					= $InstallerPaymentRequest->find('all',array('conditions'=>array('installer_id'=>decode($this->request->data['udf1']),'response_data IS NULL'),'order'=>array('id'=>'desc')))->first();
				if(!empty($arrPayment)) {
