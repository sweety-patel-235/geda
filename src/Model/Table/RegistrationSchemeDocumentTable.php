...
class RegistrationSchemeDocumentTable extends AppTable
{
	var $table = 'registration_scheme_docs';
    ...
    public function getReportData($id)
    {
        ...
        if (!empty($documents)) {
            foreach ($documents as $document) {
                $image_path = APPLYONLINE_PATH.$document['application_id'].'/registration/';
                if (file_exists($image_path.$document['filename'])) {
                    $document['url'] = APPLYONLINE_URL.$document['application_id'].'/registration/'.$document['filename'];
                    ...
                    } else if (strpos($document['filename'], 'drawing_') !== false) {
                        $arrDocuments['drawing'][] = $document;
                    }
                }
            }
        }
