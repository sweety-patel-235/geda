 ...
 */
class ReApplicationPaymentTable extends AppTable {
	/**
	 * The status of $name is universe
	 * Potential value are Class Name
	 * @var String
	 */
	public $Name 	= 'ReApplicationPayment';
	 ...
	 * @var String
	 */
	public $useTable = 're_application_payment';
	...
	public function savedata_success($arr_request_data,$is_mobile=0)
	{
		...
		if ($this->save($payusave)) 
		{
			if($payusave->udf1!='' && $payusave->payment_status=='success')
			{
				...
				$applyonlinepaySave                 = $ReSuccessPayment->newEntity();
				...
				$ReSuccessPayment->save($applyonlinepaySave);

				//Receipt  No Code
				$ReceiptMaster->save_receipt_master(decode($payusave->udf1), $receipt_no, $payu_data['max'] + 1, 1, $current_year);
				//Receipt  No Code
	...
	//	$to                         = !empty($CUSTOMER_EMAIL)?$CUSTOMER_EMAIL:"kalpak.yugtia@gmail.com";
		...
		$payment_data               = $ReSuccessPayment->find('all',array('conditions'=>array('application_id'=>$application_id),'order'=>array('id'=>'desc')))->first();
		...
		$PaymentFileName            = $Applications->generateRePaymentReceiptPdf(encode($application_id),false,true);
		//->bcc("jayshree.tailor@yugtia.com")
				
		$email          = new Email('default');
		$email->profile('default');
		$email->viewVars($EmailVars);
		$message_send   = $email->template('re_payment_receipt_mail', 'default')
				->emailFormat('html')
