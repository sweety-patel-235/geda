 ...
 */
class GeoApplicationPaymentTable extends AppTable {
	/**
	 * The status of $name is universe
	 * Potential value are Class Name
	 * @var String
	 */
	public $Name 	= 'GeoApplicationPayment';
	 ...
	 * @var String
	 */
	public $useTable = 'geo_application_payment';
	...
	public function savedata_success($arr_request_data,$is_mobile=0)
	{
		...
		if ($this->save($payusave)) 
		{
			...
			{
				$goe_location_Table                 = TableRegistry::get('ApplicationGeoLocation');
				$receipt_master 				= TableRegistry::get('ReceiptMaster');
				$goe_location_Table->updateAll(['payment_status' => '1','payment_date'=>$this->NOW()], ['id IN' => $geo_id,'application_id'=> decode($payusave->udf1)]);
				$GeoSuccessPayment 					= TableRegistry::get('GeoSuccessPayment');
				$applyonlinepaySave                 = $GeoSuccessPayment->newEntity();
				...
				$GeoSuccessPayment->save($applyonlinepaySave);
				
				$receipt_no_exist = $receipt_master->find('all',array('conditions'=>array('receipt_no'=>$payusave->receipt_no)))->first();

				if(empty($receipt_no_exist)){
					//Receipt  No Code
					$ReceiptMaster->save_receipt_master(decode($arr_request_data['merchant_param1']), $receipt_no, $payu_data['max'] + 1,3, $current_year);
					//Receipt  No Code
				}
				

				$ApplicationPaymentRequest 		= TableRegistry::get('GeoApplicationPaymentRequest');
	...
	//	$to                         = !empty($CUSTOMER_EMAIL)?$CUSTOMER_EMAIL:"kalpak.yugtia@gmail.com";
		...
		$payment_data               = $GeoSuccessPayment->find('all',array('conditions'=>array('application_id'=>$application_id),'order'=>array('id'=>'desc')))->first();
		...
		$PaymentFileName            = $Applications->generateGeoPaymentReceiptPdf(encode($payment_data->payment_id),false,true);
