...
class CheckMeterStatusShell extends Shell
{
    ...
    public function main()
    {
        ...
        $LastRowID                    = $this->CronApiProcess->GetLastRowID("check_meter_report");
        if (!empty($LastRowID)) 
        {
            $arrConditions      = [ 'application_status IN '=>$application_status,
                                    'apply_state'=>$GUJARAT_STATE,
                                    'LENGTH (consumer_no) >'=>5,
                                    'ApplyOnlines.discom NOT IN '=>array($this->ApplyOnlines->torent_ahmedabad,$this->ApplyOnlines->torent_surat,$this->ApplyOnlines->torent_dahej),
                                    'id > '=>$LastRowID];
        } else {
            $arrConditions      = [ 'application_status IN '=>$application_status,
                                    'LENGTH (consumer_no) >'=>5,
        ...
        if (!empty($FetchedRowCount))
        {
			foreach($arrApplications as $arrApplication)
			{
                ...
                $this->CronApiProcess->saveAPILog($LastProcessedApplicationID,"check_meter_report");
			}
