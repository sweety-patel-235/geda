...
<div class="container project-leads">
    ...
    <div class="text-right">
        <ul class="nav nav-pills">
            <li role="presentation" class="<?php echo ($type == "pending") ? 'active' : '' ;?>">
                <?= $this->Html->link('New', ['controller' => 'project','action' => 'leads', 'pending']) ?>
            </li>
            <li role="presentation" class="<?php echo ($type == "accepted") ? 'active' : '' ;?>">
                <?= $this->Html->link('Accepted', ['controller' => 'project','action' => 'leads', 'accepted']) ?>
            </li>
            <li role="presentation" class="<?php echo ($type == "rejected") ? 'active' : '' ;?>">
                <?= $this->Html->link('Rejected', ['controller' => 'project','action' => 'leads', 'rejected']) ?>
            </li>
            <li role="presentation" class="<?php echo ($type == "forwarded") ? 'active' : '' ;?>">
                <?= $this->Html->link('forwarded', ['controller' => 'project','action' => 'leads', 'forwarded']) ?>
            </li>
        </ul>
    </div>
    <br>
   ...
   -->
            ...
            ?>
                ...
                <div class="row p-row">
                    <div class="p-title"><?= $this->Html->link($projectLead['projects']['name'], ['action' => 'view', encode($projectLead['id'])]) ?>
                        <span class="p-date">on <?= (!empty($projectLead['projects']['created']) ? date(LIST_DATE_FORMAT,strtotime($projectLead['projects']['created'])) : ''); ?></span>
                    </div>
                    <div>Capacity: <?php echo $projectLead['projects']['capacity_kw']." KW"; ?></div>
                    <div>Location: <?php echo $projectLead['projects']['address']; ?></div>
                    <div>City: <?php echo $projectLead['projects']['city']; ?></div>
