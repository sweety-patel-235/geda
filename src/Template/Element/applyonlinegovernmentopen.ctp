				...
				<table width="100%">
				   ...
				   <tr>
					...
					<tr>
						<td class="td_simple">
							<div style="margin-top:5px">
							   Your application is registered for installation of the rooftop solar system of <?php echo $ApplyOnlines->pv_capacity; ?> kW <?php echo $pvCapacityText;?> <?php echo $pvDCText;?> (the “Capacity”) under <u>Government</u> Sector.    
							</div>
						</td>
					</tr>
					<tr>
					...
					<tr>
						<td class="td_simple">
							<div style="margin-left:15px">
								...
								<p>2. In the event if you have to change your selected Installer after the Registration, you need to cancel this Registration and apply under a fresh application and in such cases the registration fees shall not be refunded.</p>
								<p>3. Irrespective of the kW rating of the system proposed to be installed.</p>
								<p>4. You have registered the solar PV plant under the <?php if($applyOnlinesOthersData->rpo_rec == 1) {
								...
								if(empty($applyOnlinesOthersData->rpo_rec)) {
									...
									elseif($newSchemeApp == 1 && $ApplyOnlines->category != 3001) { echo '10';} 
										...
										if($applyOnlinesOthersData->renewable_attr == 1) { echo '9.1.2 (Type 1)'; } ?>
										<?php if($applyOnlinesOthersData->renewable_attr == 0) { $strPoint = 'ABT'; echo '9.1.2 (Type 2A)'; } ?>
									<?php }
								} ?>
									.</p>
								<?php
					...
					<tr>
						<td class="td_simple"> 
							<div style="margin-left:15px"> 
								<p><?php echo $sr_no; $sr_no++;?>. The energy settlement option is predefined by you as per the provisions of the Policy.</p>
								...
								<p><?php echo $sr_no; $sr_no++;?>. You shall allow access to your premises by representatives of GEDA/DISCOM/CEI for installation of <?php echo $strPoint;?> meter/inspection/verification of the system.</p>
								<?php if($applyOnlinesOthersData->rpo_rec == 1) { ?>
									<p><?php echo $sr_no; $sr_no++;?>. The Specification of ABT/Bi-Directional Meter for Metering shall be as per Provision of Policy & GERC Regulations and Prescribed by STU/Discom.</p>
								<?php } else { ?>
									<p><?php echo $sr_no; $sr_no++;?>. The bi-directional meter shall be installed in accordance to the technical specifications defined by the DISCOM.</p>
								<?php } ?>
								<p><?php echo $sr_no; $sr_no++;?>. The technical specifications of the Solar PV system and all its allied components, shall be mutually agreed between YOU and the INSTALLER selected by you.</p> 
								<p><?php echo $sr_no; $sr_no++;?>. It is YOUR sole responsibility to ensure and check all the components properly with the selected INSTALLER. In no case, GEDA shall responsible for the delay in installation, sub-standard material, low generation or any issue related to performance and quality. However, it is advisable to always install solar PV system in the shadow free area and with proper sizing and structures for maximum generation. Further, the Consumer shall check the requisite documents provided by your installer to ascertain the above mentioned points.</p> 
								<p><?php echo $sr_no; $sr_no++;?>. The Applicant, <?php echo $ApplyOnlines->name_of_consumer_applicant;?> has given following signed undertaking in the application form for installation of the solar roof top system under this registration.</p> 
								<?php if($applyOnlinesOthersData->msme == 1) { $strText = ($applyOnlinesOthersData->contract_load_more == 1) ? 'above' : 'below';?>
