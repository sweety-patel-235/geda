			...
			<div id="content" class="mainbox">
				<table width="100%">
					...
					<tr>
						<td class="td_simple">
							<div style="margin-left:15px">
								...
								<p>2. In the event if you have to change your selected Installer after the Registration, you need to cancel this Registration and apply under a fresh application</p>
								...
								if(empty($applyOnlinesOthersData->rpo_rec)) {
									...
									<?php if($applyOnlinesOthersData->renewable_attr == 0) { $strPoint = 'ABT'; echo '(Type 2A)'; } ?><?php } 
								}?>.</p>   
							</div>
						</td>
					</tr>
					<tr>
						<td class="td_simple"> 
							<div style="margin-left:15px"> 
								<p>5. You shall ensure that the location proposed for installation of the system is shadow free during the entire day and also keep the system clean and dust free for optimum output.</p>
								<p>6. You shall sign the connectivity agreement with the DISCOM upon its ascertaining the technical feasibility of the system installation.</p>
								<p>7. You shall allow access to your premises by representatives of GEDA/DISCOM/CEI for installation of <?php echo $strPoint;?> meter/inspection/verification of the system.</p>
								<?php if($applyOnlinesOthersData->rpo_rec == 1) { ?>
									<p>8. The Specification of ABT/Bi-Directional Meter for Metering shall be as per Provision of Policy & GERC Regulations and Prescribed by STU/Discom.</p>
								<?php } else { ?>
									<p>8. The bi-directional meter shall be installed in accordance to the technical specifications defined by the DISCOM.</p>
								<?php } ?>
								<p>9. The technical specifications of the Solar PV system and all its allied components, shall be mutually agreed between YOU and the INSTALLER  selected by you.</p>
								...
								<p>11. The Applicant, <?php echo $ApplyOnlines->name_of_consumer_applicant;?> has given following signed undertaking in the application form for installation of the solar roof top system under this registration.</p>
								<?php $sr_no=12; if($applyOnlinesOthersData->msme == 1) { $strText = ($applyOnlinesOthersData->contract_load_more == 1) ? 'above' : 'below';?>
