		...
		<div class="col-md-12">
			...
			<div class="form-body">
				<div class="row col-md-12">
					...
					<div class="col-md-3 ">
						<?php echo $this->Form->input('installer_name', array('label' => false ,'div'=>false,'type'=>'text','class'=>'form-control form-control-inline input-medium','id'=>'installer_name','placeholder'=>'Developer Name')); ?>
					</div>
					<?php if($authority_account == 1) { ?>
						<div class="col-md-3 ">
							<?php echo $this->Form->select('payment_status',$PAYMENT_STATUS,array('label' => false,'class'=>'form-control','empty'=>'-Select Payment Status-','style'=>'margin-left:-15px;')); ?>
						</div>
					<?php } ?>
				</div>
				<div class="row col-md-12">
...
$(document).on("click", ".approve_Status" , function() {
	...
	({
		type: "POST",
		url: "/DeveloperRegistrations/fetchDeveloper",
		data: {'developerid':insid},
...
$(".request_approval_btn").click(function(event) {
	...
	if (messagebox.length < 1 && $("#"+fromobj).find("#geda_approval").val()==2) {
		...
		$.ajax({
				type: "POST",
				url: "/DeveloperRegistrations/ApproveRegistration",
