		...
		<div class="alert alert-warning">
			...
			<ul>
			   ...
			   <li style="font-size: 1.0em;">The applications under Residential, Commercial and Industrial categories are for the "Rooftop solar PV Power plant" or "Rooftop Solar PV System" or "Solar Generating Plant"  that means the Solar Photo Voltic Power including small solar system installed on the rooftops/ ground mounted or open land of consumer premises that uses sunlight for direct conversion into electricity through photovoltaic technology.</li>
				...
				<li style="font-size: 1.0em;">It is suggested to kindly check the GST number twice before entering. The same will not be changed once the application is processed. GST credit will not be passed on to those who have not entered the correct GST number.</li>
				<li style="font-size: 1.0em;"><a href="/MNRE_Office_Memorundum_on_ALMM_28_03_2022.pdf" target="_blank">MNRE Office Memorandum on ALMM</a></li>
	...
	<div class="tabs tabs-bottom tabs-simple nk_tabs">
		...
		<div class="tab-content" >
			<div class="tab-pane <?php echo $DisplayCreateProTab;?><?php if(($tab == '' || $tab == 'tab_1') && $create_project=='1') { echo 'active'; } ?>" id="tabsNavigationSimple4" >
				...
				//I don't want subsidy on the Solar PV System.
				?>
				<div class="form-group <?php echo $class_hide;?>">
					<div class="col-md-6">
					<label style="margin-left:20px;">
						<?php echo $this->Form->input('project_disclaimer_subsidy', array('label' => false,'value'=>'1','type'=>'checkbox','class'=>'form-control check-box-address','placeholder'=>'','id'=>'project_disclaimer_subsidy',"checked"=>$checked)); ?>I am applying for Gujarat Renewable Energy Policy – 2023. <?php //Solar PV System under Gujarat Solar Policy 2021. ?>
						</label>
					</div>
				</div>
				<?php
				if($allocatedCategory==3)
				{
					...
					<div class="form-group">
						<div class="col-md-12 oneline ">
							<label>
								<i class="fa fa-check"></i> I am applying for Gujarat Renewable Energy Policy – 2023. <?php //Solar PV System under Gujarat Solar Policy 2021 ?>
							</label>
						</div>
					</div>
					<?php
			...
			<div class="tab-pane <?php echo $DisplayInstallershowTab;?><?php if($tab == 'tab_1' && $create_project=='0'){ echo 'active'; } ?>" id="tabsNavigationSimple2">
				<div id="discomdata">
					...
					<div class="form-group">
						<div class="col-md-4">
							...
							{
								$approval   = $MStatus->Approvalstage($ApplyOnlines->id);
								if(in_array($MStatus->APPLICATION_SUBMITTED,$approval) && $is_installer == true)
								{
									$isReadonly     = true;
								}
							}
							?>
							<label>PV Capacity (<?php echo $pvCapacityText;?>) to be installed (in kW)<span class="mendatory_field">*</span></label>
							<?php echo $this->Form->input('ApplyOnlines.pv_capacity', array('label' => false,'class'=>'form-control','id'=>'pv_capacity','placeholder'=>'PV Capacity','onkeypress'=>"check_pv_cap();return validateDecimal(event)",'onblur'=>'javascript:check_pv_cap();','readonly'=>$isReadonly)); ?>
						</div>
						...
						<?php if($newSchemeApp == 1) { ?>
							<div class="col-md-4">
								...
								<?php echo $this->Form->input('ApplyOnlines.pv_dc_capacity', array('label' => false,'class'=>'form-control','placeholder'=>'Plant DC Capacity','onkeypress'=>"return validateDecimal(event)",'readonly'=>$isReadonly)); ?>
							</div>
						<?php $class 	= 'col-md-4';} ?>
				...
				<?php if(!empty($installer_list) && ($ses_login_type == 'developer' || ($ses_login_type == 'member' && (isset($ApplyOnlinesOthers->map_installer_id) && !empty($ApplyOnlinesOthers->map_installer_id))))) { 
				//	$sel_developer_id 	= (isset($ApplyOnlinesOthers->map_installer_id) && !empty($ApplyOnlinesOthers->map_installer_id)) ? encode($ApplyOnlinesOthers->map_installer_id) : '';
					?>
					<h4><u> Installer Detail</u></h4>
					<div class="form-group">
						<?php 
							$error_class_installer 	= '';
							if(isset($ApplyOnlineErrors['map_installer_id']) && isset($ApplyOnlineErrors['map_installer_id']['_empty']) && !empty($ApplyOnlineErrors['map_installer_id']['_empty'])) { $error_class_installer = 'has-error'; }?>
							<div class="col-md-4 <?php echo $error_class_installer;?>">
								<?php //echo $this->Form->select('ApplyOnlines.developer_id',$developer_list, array('label' => false,'class'=>'form-control chosen-select','empty'=>'-Select Developer-','id'=>'developer_id','value'=>$sel_developer_id)); ?>
								<?php echo $this->Form->select('ApplyOnlines.map_installer_id',$installer_list, array('label' => false,'class'=>'form-control chosen-select','placeholder'=>'-Select Installer-','empty'=>'-Select Installer-')); ?>
								<?php  if(!empty($error_class_installer)) {  ?>
									<div class="help-block">Please select Installer</div>
								<?php } ?>
							</div>
						<?php /*<div class="col-md-2">
							<?php echo $this->Form->input('Search Developer', array('label' => false,'class'=>'next btn btn-primary btn-md mb-xlg search','id'=>'search_data_btn_dev','onclick'=>'javascript:click_search_developer();','type'=>'button','templates' => ['inputContainer' => '{{content}}'])); ?>
						</div>
						<div class="col-md-2">
							<?php echo $this->Form->input('Map Developer', array('label' => false,'class'=>'next btn btn-primary btn-md mb-xlg search','id'=>'map_data_btn_dev','onclick'=>'javascript:click_map_developer();','type'=>'button','templates' => ['inputContainer' => '{{content}}'])); ?>
						</div>
						<div class="col-md-1" id="spinner_dev" style="display: none;" >
								<i class="fa fa-circle-o-notch fa-spin" style="font-size:24px; margin-top: 14px;color:#71bf57"></i>
								<?php 
									echo $this->Form->input('ApplyOnlines.map_installer_id', array('id'=>'map_installer_id','label' => false,'type'=>'hidden','class'=>'form-control'));
								?>
							</div> */?>
					</div>
					<div class="row form-group">&nbsp;</div>
					<div class="row form-group" id="developer_details" style="display: none;" >
				...
				<div class="form-group">
					...
					<?php
						...
						$insText 	= ($ses_login_type == 'developer') ? 'Developer' : 'Installer';
					...
					<div class="col-md-3 <?php echo $error_class_for_ins_prefix; ?>">
						<label><?php echo $insText;?> Email<span class="mendatory_field">*</span></label>
						<?php echo $this->Form->input('ApplyOnlines.installer_email', array('label' => false,'class'=>'form-control','placeholder'=>$insText.' Email'));
					...
					<div class="col-md-3">
						<label><?php echo $insText;?> Mobile<span class="mendatory_field">*</span></label>
						<?php echo $this->Form->input('ApplyOnlines.installer_mobile', array('label' => false,'class'=>'form-control','placeholder'=>$insText.' Mobile',"onkeyup"=>"if (/\D/g.test(this.value)){ this.value = this.value.replace(/\D/g,'');}","maxlength"=>"10")); ?>
					</div>
				</div>
				<div class="form-group">
					<?php
				...
				<?php  $classIns 	= ($ses_login_type == 'installer') ? 'hide' : '';
				$classIns 	= ($ses_login_type == 'developer' || ($ses_login_type == 'member' && (isset($ApplyOnlinesOthers->map_installer_id) && !empty($ApplyOnlinesOthers->map_installer_id)))) ? '' : 'hide';
				 ?>
				<?php 
						$error_f_class = '';
						if(isset($ApplyOnlineErrors['ppa_doc']) && isset($ApplyOnlineErrors['ppa_doc']['_empty']) && !empty($ApplyOnlineErrors['ppa_doc']['_empty']))
						{
							$error_f_class  = 'has-error';
						}
						$hiddenVal = '';
						?>
				<div class="form-group <?php echo $classIns;?>">
					<div class="col-md-4 <?php echo $error_f_class;?>">
						<label>PPA Document <span class="mendatory_field">*</span></label>
						<?php echo $this->Form->input('ApplyOnlines.file_ppa_doc', array('label' => false,'type'=>'file','id'=>'file_ppa_doc','class'=>'form-control'));
						?>
						<div id="file_ppa_doc-file-errors"></div>
						<?php 
						if(!empty($ApplyOnlinesOthers->ppa_doc)) { ?>
							<?php if ($Couchdb->documentExist($ApplyOnlines->id,$ApplyOnlinesOthers->ppa_doc)) { ?>
									<?php
										echo "<br/> <strong><a target=\"_blank\" href=\"".URL_HTTP.'app-docs/ppa_doc/'.encode($ApplyOnlines->id)."\">View PPA Document</a></strong>";
										$hiddenVal = $ApplyOnlinesOthers->ppa_doc;
									?>
							<?php } ?>
						<?php } ?>
					
						<?php echo $this->Form->input('ApplyOnlines.ppa_doc', array('label' => false,'type'=>'hidden','id'=>'ppa_doc','class'=>'form-control','value'=>$hiddenVal)); ?>
						<?php if(!empty($error_f_class))
							{
								?>
								<div class="help-block"><?php echo $ApplyOnlineErrors['ppa_doc']['_empty']; ?></div>
								<?php
							} ?>
					</div>
					<?php 
						$error_f_class 		= '';
						if(isset($ApplyOnlineErrors['agreement_customer']) && isset($ApplyOnlineErrors['agreement_customer']['_empty']) && !empty($ApplyOnlineErrors['agreement_customer']['_empty']))
						{
							$error_f_class  = 'has-error';
						}
						$hiddenVal = '';
						?>
					<div class="col-md-4 <?php echo $error_f_class;?>">
						<label>Agreement between Customer <span class="mendatory_field">*</span></label>
						<?php echo $this->Form->input('ApplyOnlines.file_agreement_customer', array('label' => false,'type'=>'file','id'=>'file_agreement_customer','class'=>'form-control'));
						?>
						<div id="file_agreement_customer-file-errors"></div>
						<?php if(!empty($ApplyOnlinesOthers->agreement_customer)) { ?>
							<?php if ($Couchdb->documentExist($ApplyOnlines->id,$ApplyOnlinesOthers->agreement_customer)) { ?>
									<?php
										echo "<br/> <strong><a target=\"_blank\" href=\"".URL_HTTP.'app-docs/agreement_customer/'.encode($ApplyOnlines->id)."\">View Agreement between Customer</a></strong>";

									?>
							<?php } ?>
						<?php } ?>
						<?php echo $this->Form->input('ApplyOnlines.agreement_customer', array('label' => false,'type'=>'hidden','id'=>'agreement_customer','class'=>'form-control','value'=>$hiddenVal)); ?>
						<?php if(!empty($error_f_class))
							{
								?>
								<div class="help-block"><?php echo $ApplyOnlineErrors['agreement_customer']['_empty']; ?></div>
								<?php
							} ?>
					</div>
				</div>
				...
				<div class="col-md-4 hide">
					...
					<br/>
					<div class="file-loading" >
						<?php echo $this->Form->input('ApplyOnlines.app_upload_undertaking', array('label' => false,'div' => false,'type'=>'file','id'=>'app_upload_undertaking','templates' => ['inputContainer' => '{{content}}'])); ?>
					</div>
					<br/><a href="/undertaking" target="_blank">[Download Undertaking Format]</a>
					<div id="app_upload_undertaking-file-errors"></div>
					<?php
					if(!empty($ApplyOnlinesOthers->upload_undertaking)) :  ?>
						<?php if ($Couchdb->documentExist($ApplyOnlinesOthers->application_id,$ApplyOnlinesOthers->upload_undertaking)) { 
							echo "<br/><strong><a target=\"_blank\" href=\"".URL_HTTP.'app-docs/app_upload_undertaking/'.encode($ApplyOnlinesOthers->application_id)."\">View Undertaking form</a></strong>";
						 	} 
						?>
					<?php endif; ?>
				</div>
				...
				<div class="form-group <?php echo $class_hide;?>">
					<div class="col-md-12 oneline ">
						...
						if($flg_disp == '1')
						{
						 ...
						 ?>
							<?php echo ($ApplyOnlinesOthers->scheme_id == 3) ? "I am applying for Gujarat Renewable Energy Policy – 2023." : (($newSchemeApp == 0) ? "I don't want subsidy on the Solar PV System." : "I am applying for Solar PV System under Gujarat Solar Policy 2021."); //?>
						</label>
					</div>
				</div>
				<?php
				if($allocatedCategory==3)
				{
					...
					<div class="form-group">
						<div class="col-md-12 oneline ">
							<label>
								<i class="fa fa-check"></i> <?php echo ($ApplyOnlinesOthers->scheme_id == 3) ? "I am applying for Gujarat Renewable Energy Policy – 2023." : (($newSchemeApp == 0) ? "I don't want subsidy on the Solar PV System." : "I am applying for Solar PV System under Gujarat Solar Policy 2021.");//."; ?>
							</label>
						</div>
					</div>
					<?php
				...
				<div class="">
					...
					<div class="col-md-5 div_contract_load_more hide">
						...
						<?php if(!empty($ApplyOnlinesOthers->upload_certificate)) : ?>
							<?php if($Couchdb->documentExist($ApplyOnlines->id,$ApplyOnlinesOthers->upload_certificate)) { ?>
								<?php
...
$(document).ready(function() {
	...
	$("#file_ppa_doc").fileinput({
		showUpload: false,
		showPreview: false,
		dropZoneEnabled: false,
		mainClass: "input-group-md",
		allowedFileExtensions: ["pdf"],
		elErrorContainer: '#file_ppa_doc-file-errors',
		maxFileSize: '1024',
	});
	$("#file_agreement_customer").fileinput({
		showUpload: false,
		showPreview: false,
		dropZoneEnabled: false,
		mainClass: "input-group-md",
		allowedFileExtensions: ["pdf"],
		elErrorContainer: '#file_agreement_customer-file-errors',
		maxFileSize: '1024',
	});
	
	$("#app_upload_undertaking").fileinput({
		showUpload: false,
		showPreview: false,
		dropZoneEnabled: false,
		mainClass: "input-group-md",
		allowedFileExtensions: ["pdf"],
		elErrorContainer: '#app_upload_undertaking-file-errors',
		maxFileSize: '1024',
	});
...
function click_search()
{
	...
	if(($("#consumer_no").val()).length>=3 && $("#discom").val()>0)
	{
		...
		else
		{
			...
			$.ajax({
				type: "POST",
				url: "/ApplyOnlines/getSubDivisionConsumer",
...
{
	if(result.data.arrCategory.length>1) {
		$("#category").html("");
		$("#category").append($("<option />").val('').text('-Select Category-'));
		$.each(result.data.arrCategory, function(index, title, arrCategory) {
			//var cat_text = arrCategory[title];
			console.log(arrCategory);
			//alert(index+ " "+title);
			$("#category").append($("<option />").val(title).text(index));
		});
	}
...
$('.form_submit').submit(function(){
	...
	/*if($("#developer_id").val() !== '') {
		swal({
			title: "Are you sure you want to map developer with this application?",
			text: "",
			type: "warning",
		...
		function(isConfirm) {
			if (isConfirm) {
				swal("Ok", "", "success");
				$("#sel_developer_id").val($("#developer_id").val());
				$('.form_submit').submit();
			} else {
				$("#developer_id").val('').trigger("chosen:updated");
...
function click_search_developer()
{
	//$('.chosen-select').chosen().val(1);
	
	/*if($("#developer_id").val() !=='')
	{*/
		//$("#map_installer_id").val('');
		$("#spinner_dev").show();
		$("#dev_name").html('');
		$("#dev_con_name").html('');
		$("#dev_mobile").html('');
		...
		$.ajax({
			type: "POST",
			url: "/DeveloperRegistrations/fetchDeveloperDetails",
			...
			success: function(response) {
				$("#spinner_dev").hide();
				$("#developer_details").show();
				var result = $.parseJSON(response);
				console.log(result.type);
				if(result.type=='ok')
				{
					$("#dev_name").html(result.response['developer_name']);
					$("#dev_con_name").html(result.response['developer_contact_name']);
					$("#dev_mobile").html(result.response['mobile']);
					$("#dev_email").html(result.response['email']);
					$("#dev_state").html(result.response['state']);
					$("#dev_city").html(result.response['city']);
				}
				else
				{
...
<?php /*if(!empty($developer_list)) { ?>
click_search_developer();
<?php }*/ ?>
function click_map_developer() {
	swal({
		title: "Are you sure map this developer with this application?",
		text: "",
		type: "warning",
		showCancelButton: true,
		confirmButtonClass: "btn-danger",
	...
	function(isConfirm) {
		if (isConfirm) {
			$("#map_installer_id").val($("#developer_id").val());
			swal("Ok", "", "success");
		} else {
			$("#"+actionPass).removeAttr('checked');
			swal("Cancelled", "", "error");
