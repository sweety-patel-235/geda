					...
					<div class="form-group">
						...
						<?php if($newSchemeApp == 1) { ?>
							<div class="col-md-4">
								...
								<?php echo $this->Form->input('ApplyOnlines.existing_ac_capacity', array('label' => false,'class'=>'form-control','placeholder'=>'Existing AC Capacity','onkeypress'=>"return validateDecimal(event)",'readonly'=>$isReadonly)); ?>
							</div>
						<?php } ?>
				...
				<div class="col-md-4 hide">
					...
					if(!empty($ApplyOnlinesOthers->upload_undertaking)) :  ?>
						<?php if ($Couchdb->documentExist($ApplyOnlinesOthers->application_id,$ApplyOnlinesOthers->upload_undertaking)) { 
							echo "<br/><strong><a target=\"_RECENTBILL\" href=\"".URL_HTTP.'app-docs/upload_undertaking/'.encode($ApplyOnlinesOthers->application_id)."\">View Undertaking form</a></strong>";
						 	} 
						?>
					<?php endif; ?>
				</div>
...
//$("#map_installer_id").val('761').trigger("chosen:updated");
...
function click_map_developer() {
	...
	function(isConfirm) {
		if (isConfirm) {
			$("#map_developer_id").val($("#developer_id").val());
			swal("Ok", "", "success");
		} else {
			$("#"+actionPass).removeAttr('checked');
			swal("Cancelled", "", "error");
