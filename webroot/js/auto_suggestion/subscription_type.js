 ...
 */
$('.subscription_type').live('focus',function(){
    $(this).autocomplete({
	   source: function( request, response ){
            $.ajax({
              url : WEB_ADMIN_URL+'subscriptiontypes/subscriptiontypelist/'+request.term,
              dataType: "json",
              /*data: {
    ...
    select: function( event, ui ) {
            ...
            $(this).parents('div').find('#subscription_type_id').val(names[1]);
            /*var obj=$(this);
            var dt;
            $.ajax({
